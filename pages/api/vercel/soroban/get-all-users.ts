...
try {
  ...
  if (StellarSdk.rpc && StellarSdk.rpc.Server) {
    ...
    console.log('✅ Get all users route: Soroban RPC server initialized successfully')
  } else {
    console.error('❌ Get all users route: StellarSdk.rpc.Server not found in SDK')
    throw new Error('Stellar SDK rpc.Server not available')
  }
} catch (error) {
  console.error('❌ Get all users route: Failed to initialize Soroban server:', error)
}
